...
def main():
    parser = argparse.ArgumentParser(
        description="Obfuscate or deobfuscate the contents of the file in a very simple way.")
    parser.add_argument("input", type=str, nargs="?", help="Input file path.")
    parser.add_argument("output", type=str, nargs="?",
                        help="Output file path.")
    parser.add_argument("-p", "--prompt", action="store_true",
                        help="Prompt mode. If provided, you will be prompted for the arguments you don't explicitly set.")
    parser.add_argument("-r", "--reverse", action="store_true",
                        help="If provided, the protection will be reversed. (Deobfuscate)")
    parser.add_argument("-k", "--key", type=str, default=default_key,
                        help="XOR key for encryption/decryption. Must be a valid byte string.")
    parser.add_argument("-s", "--salt", type=int, default=default_salt_sz,
                        help="Number of bytes to use in salt. (Default: 16)")
    parser.add_argument("--binary", action="store_true",
                        help="Disable hexadecimal encoding/decoding, meaning the data will be (or is) compressed binary data.")
    ...
    if args.prompt:
        ...
        output_path = output_path if output_path else input(
            "Output file path: ")
        if not reverse:
            _reverse = input(
                "Are you reversing the obfuscation? [y/n, default = n]: ").lower()
            reverse = len(_reverse) > 0 and _reverse[:1].lower() == "y"
        if not binary:
            _binary = input(
                "Should the output be hex encoded? [y/n, default = y]: ").lower()
            binary = len(_binary) > 0 and _binary[:1].lower() == "n"
        if key == default_key:
            _key = input(
                "Enter a custom encryption key, or leave blank to use default: ")
